 ...
 */
infix fun Set.union(set: Set): Set = {it -> this.contains(it) || set.contains(it)}

/**
 * Intersection of the two sets
 */
infix fun Set.intersect(set: Set): Set = {it -> this.contains(it) && set.contains(it)}

/**
 * Difference of two given sets
 */
infix fun Set.diff(set: Set): Set = {it -> this.contains(it) && !set.contains(it)}

/**
 * Filter
 */
fun Set.filter(predicate: (Int) -> Boolean): Set = this intersect predicate
...
fun Set.iterateFun(): List<Int>  {
    tailrec fun iter(x: Int, list:List<Int>): List<Int> =
      when {
      x > BOUND -> list
      else -> iter(x+1, if (invoke(x)) list+listOf(x) else list)}
    return iter(-BOUND, listOf())
}

val Set.contents: String get() = this.iterateFun().joinToString(prefix = "{", postfix = "}", separator = ",")
